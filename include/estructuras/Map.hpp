...
#include <iostream>
#include <estructuras/DLL.hpp>
#include <estructuras/Couple.hpp>
...
class Map{
	...
	public:
		 ...
		 *
		 *@return true si se inserto correctamente en el map, en caso contrario retorna false
		 */
		bool insert(Key _key,Value _value);
		...
		/**
		 *@brief Método que recorre todo el map aplicando una función a cada elemento
		 */
...
template<typename Key, typename Value>
bool Map<Key,Value>::insert(Key _key,Value _value){

	/*Creamos la couple a insertar*/
	return map ->InsertBack(new Couple<Key,Value>(_key,_value));
}
